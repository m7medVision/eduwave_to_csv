...
export default function App() {
  ...
  function convertToCsv() {
    ...
    for (const match of matchs) {
      const teacherRegex =
        /<br><\/td><td>(.*?)<\/td><td><\/td><td><\/td><td><\/td>/gms;
      const teacher = [...match[1].matchAll(teacherRegex)]?.[0]?.[1];
      const titleRegex = /<span[^>]*title="Course Name"[^>]*>(.*?)<\/span/gms;
      const courseName = [...match[1].matchAll(titleRegex)]?.[0]?.[1];
      const timeTableRegexUncleaned =
        /\((\d{2}\/\d{2}\/\d{4})\)[^>]*(\d{2}:\d{2}:\d{2})[^>]*-[^>]*(\d{2}:\d{2}:\d{2})(.*?)<br/gms;
      const timeTableUncleaned = [
        ...match[1].matchAll(timeTableRegexUncleaned)
      ];
      interface TimeTable {
        date: string;
        startTime: string;
    ...
    <div className="flex h-full flex-col items-center justify-center gap-4 px-4">
      <section className="flex flex-col items-center gap-1 text-center">
        <h1 className="text-3xl font-semibold">
          Convert eduwave timetable to csv
        </h1>
        <p className="text-sm font-medium">
          convert eduwave timetable to csv that can be imported to google
          calendar
        </p>
      </section>
      <InputFile />
      <Button className="" onClick={convertToCsv}>
        Convert
      </Button>
    </div>
  );
